(function () {
	...
	function applyTheme() {
		...
		window.dispatchEvent(
			new CustomEvent("dsa-theme-change", {
				detail: {
					theme: currentTheme,
				},
			})
	...
	function injectStyles() {
		if (document.getElementById("dsa-theme-toggle-styles")) {
			return;
		}
		...
		style.textContent =
			...
			"border-color: var(--accent, #f5a800);" +
			"transform: translateY(-1px);" +
			"}" +

			".dsa-theme-toggle:active {" +
			"transform: translateY(0) scale(0.96);" +
			"}" +

			".dsa-theme-toggle:focus-visible {" +
			"outline: 2px solid var(--accent, #f5a800);" +
			"outline-offset: 3px;" +
			"}" +

			".dsa-theme-toggle-track {" +
			...
			"border-radius: 50%;" +
			"background: linear-gradient(135deg, #ffcd39, #f5a800);" +
			"color: #1a1400;" +
			...
			".dsa-theme-toggle-thumb svg {" +
			"position: absolute;" +
			"width: 13px; height: 13px;" +
			"transition: opacity 0.25s ease, transform 0.35s ease;" +
			"}" +

			".dsa-theme-toggle-thumb .icon-moon {" +
			"opacity: 0;" +
			"transform: rotate(-70deg) scale(0.4);" +
			"}" +

			".dsa-theme-toggle-thumb .icon-sun {" +
			"opacity: 1;" +
			"transform: rotate(0deg) scale(1);" +
			"}" +

			".dsa-theme-toggle.is-dark .dsa-theme-toggle-thumb .icon-sun {" +
			"opacity: 0;" +
			"transform: rotate(70deg) scale(0.4);" +
			"}" +

			".dsa-theme-toggle.is-dark .dsa-theme-toggle-thumb .icon-moon {" +
			"opacity: 1;" +
			"transform: rotate(0deg) scale(1);" +
			"}" +
	...
	function init(opts) {
		...
		if (!opts.hideToggle && isDsaThemePage()) {
			injectButton();
		}
