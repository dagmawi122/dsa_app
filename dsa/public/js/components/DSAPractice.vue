<template>
	...
	<div v-else-if="!problem" class="dsa-state">
		{{ __("No problems have been published yet.") }}
	</div>
		...
		<div class="dsa-panels" :style="panelStyle">
			<div class="dsa-pane dsa-problem-pane">
				...
				<div v-else class="dsa-tab-content">
					...
					<div v-if="props.contestMode && contestProgress" class="contest-progress">
						...
						<template v-else>
							...
							<div v-else-if="testResults.length" class="dsa-results">
								...
								<div v-if="activeTestResult" class="dsa-result-details">
									...
									<label v-if="activeTestResult.expected_output !== undefined">
										{{ __("Expected Output") }}
									</label>

									<pre v-if="activeTestResult.expected_output !== undefined">
                                        {{ activeTestResult.expected_output || __("No output") }}
                                    </pre>

									<label>
										{{ activeTestResult.error ? __("Error") : __("Output") }}
									</label>
...
async function loadProblem(name) {
	...
	problem.value = await call("dsa.api.get_problem", {
		name,
	});
...
async function submitCode() {
	...
	try {
		const method = props.contestMode ? "dsa.api.submit_contest_code" : "dsa.api.submit_code";
		...
		const queued = await call(method, args, "POST");
		...
		for (let attempt = 0; attempt < 60 && generation === currentGeneration; attempt += 1) {
			const result = await call(
				...
				{
					submission: props.contestMode ? queued.contest_submission : queued.submission,
				}
			);
